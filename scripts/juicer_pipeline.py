...
for mergeGroupIdx in mergedSamples.index:
	...
	#make juicer directory for merge group
	...
	for sampleIdx in mergeDataframe.index:
		...
		read1_dest = scratchDir+"/"+mergeName+"/fastq/"+os.path.basename(read1_source)
		read2_dest = scratchDir+"/"+mergeName+"/fastq/"+os.path.basename(read2_source)
	...
	os.system("./juicer/scripts/juicer.sh " + scratchDir + "/" + mergeName + "/" + mergeName + "_samplesheet.txt " + finalDir + " " + juicerOptions)
